...
potlevels = np.arange(-55, 55, 1)+2.5
potmin, potmax = -52.5, 52.5
Ncontour = 7
...
for tilt in tilts:#tilt = tilts[0]
    ...
    for i in range(3):
        for j in range(3):
            ...
            phinmax, phinmin = phin[OKindsn].max(),phin[OKindsn].min()
            phismax, phismin = phis[OKindss].max(),phis[OKindss].min()
            dPhiN = phinmax - phinmin
            dPhiS = phismax - phismin
            ...
            #tmpCPCP = np.max([phinmax, phismax]) - np.min([phinmin, phismin])
            #dlevel = tmpCPCP/Ncontour
            #tmplevels = np.arange(potmin,potmax, dlevel)
            # if np.abs(tmplevels[-1]+dlevel-potmax) < np.abs(tmplevels[-1]-potmax):
            #     tmplevels = np.append(tmplevels,tmplevels[-1]+dlevel)
            tmpMAX = np.max(np.abs(np.array([phinmax,phismax,phinmin, phismin])))
            ...
            #           'By':By,
            #           'Bz':Bz}
            # total_fac = total_fac.append(pd.DataFrame(facdat, index = [(_ca, tilt)]))

            # total_fac = total_fac.append(pd.DataFrame({'north':j_up_n, 'south':j_up_s}, index = [(_ca, tilt)]))
    ...
    if no_SH:
        figname = plotdir+'swipe_potential_ns_comparison_' + tiltstr + f'_deg_tilt_noSH__{Ncontour}contours.png'
    else:
        figname = plotdir+'swipe_potential_ns_comparison_' + tiltstr + f'_deg_tilt__{Ncontour}contours.png'
    # if switchem:
